...
Future<void> run(WidgetTester tester) async {
  ...
  debugPrint('TEST_LOG: [LOGIN] Phase Start');

  FlavorConfig.flavor = Flavor.dev;

  // 1. Sync dependencies
  debugPrint('TEST_LOG: [LOGIN] di.init()');
  await tester.runAsync(() async {
    await di.init();
  });

  // 2. Launch UI
  debugPrint('TEST_LOG: [LOGIN] pumpWidget(MyApp)');
  ...
  debugPrint('TEST_LOG: [LOGIN] Pulsing engine frames...');
  for (int i = 0; i < 5; i++) {
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 200));
  }

  debugPrint('TEST_LOG: [LOGIN] Searching for UI elements...');
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (tester.any(usernameField)) {
      found = true;
      debugPrint('TEST_LOG: [LOGIN] UI Found at iteration $i');
      break;
    }
  }

  if (!found) {
    debugPrint('TEST_LOG: ERROR - Login UI never appeared');
    return;
  }

  // 4. Credentials
  debugPrint('TEST_LOG: [LOGIN] Entering Credentials');
  ...
  debugPrint('TEST_LOG: [LOGIN] Submitting Form');
  ...
  for (int i = 0; i < 30; i++) {
    ...
    if (tester.any(logoutIcon)) {
      ...
      debugPrint('TEST_LOG: [LOGIN] Dashboard reached');
      break;
    }
  }

  if (dashboardReached) {
    debugPrint('TEST_LOG: [LOGIN] Success');
  } else {
    debugPrint('TEST_LOG: ERROR - Login Failed to reach Dashboard');
  }
}
